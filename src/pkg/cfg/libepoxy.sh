...
# Mandatory variables
declare -rg CFG_PKG_NAME='libepoxy'
declare -rg CFG_VERSION='1.5.10'
declare -rg CFG_WEBSITE='https://github.com/anholt/libepoxy'
...
function cfg_post_install() {
  for pc in "$PKG_INS"/lib{,64}/pkgconfig/epoxy.pc
  do
    [[ -f "$pc" ]] || continue
    sed -i "s/Cflags:/Cflags: -DEPOXY_${BKIND^^}/" "$pc"
  done
}
