...
# Mandatory variables
declare -rg CFG_PKG_NAME='nghttp2'
declare -rg CFG_VERSION='1.69.0'
declare -rg CFG_WEBSITE='https://nghttp2.org/'
...
function cfg_post_build() {
  ...
  then
    sed -i '/^Cflags:/ s/$/ -DNGHTTP2_STATICLIB/' "$PKG_BLD/lib/libnghttp2.pc"
  fi
}
